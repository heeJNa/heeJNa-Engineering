#!/bin/bash
# =============================================================================
# Quality Gate Hook - Nuxt 4
# =============================================================================
# Stop Hook: Claude Code가 작업을 마치기 전 자동으로 ESLint + TypeCheck 실행
# =============================================================================
...
# -----------------------------------------------------------------------------
# Nuxt TypeCheck (.ts, .vue 변경 시)
# -----------------------------------------------------------------------------
TS_FILES=$(echo "$CHANGED_FILES" | grep -E '\.(ts|vue)$' || true)

if [ -n "$TS_FILES" ]; then
  NUXT_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)

  if [ -f "$NUXT_ROOT/nuxt.config.ts" ] || [ -f "$NUXT_ROOT/nuxt.config.js" ]; then
    if command -v npx &>/dev/null; then
      TC_OUTPUT=$(run_with_timeout 60 npx nuxi typecheck 2>&1) || {
        if [ $? -ne 124 ]; then
          ERRORS="${ERRORS}\n[TypeCheck] 타입 에러 발견:\n${TC_OUTPUT}\n"
        fi
      }
